...
task :sync do
  ...
  # sync css / js files
  system "aws --profile=dixonandmoe s3 sync gzip/ s3://#{bucket}/ --region us-east-1 --acl=public-read --delete --cache-control='max-age=2629000' --content-encoding='gzip' --exclude '*' --include '*.css' --include '*.js'"

  # sync html / xml files
  system "aws --profile=dixonandmoe s3 sync gzip/ s3://#{bucket}/ --region us-east-1 --acl=public-read --delete --cache-control='max-age=0, no-cache' --content-encoding='gzip' --exclude '*' --include '*.html' --include '*.xml' --include '*.txt'"

  # sync image files
  system "aws --profile=dixonandmoe s3 sync gzip/ s3://#{bucket}/ --region us-east-1 --acl=public-read --delete --cache-control='max-age=2629000' --exclude '*' --include 'assets/images/*' --exclude 'assets/images/*.ico' --include '*.json'"
